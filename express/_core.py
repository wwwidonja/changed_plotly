...
def set_mapbox_access_token(token):
    """
    Arguments:
        token: A Mapbox token to be used in `new_plotly.express.scatter_mapbox` and \
        `new_plotly.express.line_mapbox` figures. See \
...
def get_trendline_results(fig):
    ...
    Arguments:
        fig: the output of a `new_plotly.express` charting call
    Returns:
...
def apply_default_cascade(args):
    ...
    # load the default template if set, otherwise "new_plotly"
    if args["template"] is None:
        if pio.templates.default is not None:
            args["template"] = pio.templates.default
        else:
            args["template"] = "new_plotly"
        ...
        for i, (argument, field) in enumerate(zip(argument_list, field_list)):
            ...
            if isinstance(argument, pd.MultiIndex):
                raise TypeError(
                    ...
                    "pandas MultiIndex is not supported by new_plotly express "
                    "at the moment." % field
                )
