...
def to_html(
    ...
    include_plotlyjs: bool or string (default True)
        ...
        in the output. The url used is versioned to match the bundled new_plotly.js.
        ...
        internet connection in order to load the new_plotly.js library.
...
""".format(
    ...
    # ## Handle loading/initializing new_plotly.js ##
    ...
    # Init plotlyjs. This block needs to run before new_plotly.js is loaded in
    # order for MathJax configuration to work properly
    if include_plotlyjs == "require":
        require_start = 'require(["new_plotly"], function(Plotly) {'
        require_end = "});"
    ...
    """.format(
        ...
        load_plotlyjs = """\
        {win_config}
        <script src="new_plotly.min.js"></script>\
    """.format(
...
<div>\
        {mathjax_script}\
        {load_plotlyjs}\
            <div id="{id}" class="new_plotly-graph-div" \
...
def write_html(
    ...
    # Check if we should copy new_plotly.min.js to output directory
    if path is not None and full_html and include_plotlyjs == "directory":
        bundle_path = path.parent / "new_plotly.min.js"
