...
class OrcaConfig(object):
    ...
    These parameters may optionally be saved to the user's ~/.new_plotly
    ...
    def __init__(self):
        ...
        # Compute absolute path to the 'new_plotly/package_data/' directory
        ...
        # Compute constants
        plotlyjs = os.path.join(self.package_dir, "new_plotly.min.js")
        self._constants = {
            "plotlyjs": plotlyjs,
...
but received value of type {typ}l
    ...
    def reload(self, warn=True):
        """
        Reload orca settings from ~/.new_plotly/.orca, if any.

        Note: Settings are loaded automatically when new_plotly is imported.
...
The server_url property must be a string, but received value of type {typ}.
    ...
    def port(self):
        ...
        new_plotly.io.orca.status.port property.
...
The port property must be an integer, but received value of type {typ}.
    ...
    def executable(self):
        ...
        new_plotly.io.orca.status.executable property.
...
The timeout property must be a number, but received value of type {typ}.
    ...
    def default_width(self):
        ...
        applied if no width value is supplied to the new_plotly.io
...
The default_width property must be an int, but received value of type {typ}.
    ...
    def default_height(self):
        ...
        applied if no height value is supplied to the new_plotly.io
        to_image or write_image functions.
...
The default_height property must be an int, but received value of type {typ}.
    ...
    def default_format(self):
        ...
        new_plotly.io to_image or write_image functions.
...
The use_xvfb property must be one of {valid_vals}
    ...
    def plotlyjs(self):
        """
        The new_plotly.js bundle being used for image rendering.
    ...
    def config_file(self):
        ...
        Using the `new_plotly.io.config.save()` method will save the current
...
class OrcaStatus(object):
    ...
    def executable(self):
        ...
        property of the `new_plotly.io.orca.config` object.
...
def validate_executable():
    ...
    `new_plotly.io.orca.config.executable` property.

    If the `new_plotly.io.orca.status.state` property is 'validated' or 'running'
    then this function does nothing.

    How it works:
      - First, it searches the system PATH for an executable that matches the
      name or path specified in the `new_plotly.io.orca.config.executable`
      property.
      - Then it runs the executable with the `--help` flag to make sure
      it's the new_plotly orca executable
...
If you haven't installed orca yet, you can do so using conda as follows:
    ...
    $ conda install -c new_plotly new_plotly-orca
...
If you have installed orca, then for some reason new_plotly.py was unable to
locate it. In this case, set the `new_plotly.io.orca.config.executable`
property to the full path of your orca executable. For example:

    >>> new_plotly.io.orca.config.executable = '/path/to/orca'

After updating this executable property, try the export operation again.
If it is successful then you may want to save this configuration so that it
will be applied automatically in future sessions. You can do this as follows:

    >>> new_plotly.io.orca.config.save()
...
{instructions}""".format(
    ...
    if config.use_xvfb == True:
        ...
        if not xvfb_run_executable:
            raise ValueError(
                """
The new_plotly.io.orca.config.use_xvfb property is set to True, but the
...
does not seem to be a valid new_plotly orca executable. Please refer to the end of
...
detected. Please install Xvfb and configure new_plotly.py to run orca using Xvfb
as follows:
...
Here is the command that new_plotly.py ran to request the version
    $ {executable} --version
...
""".format(
            ...
            if pid_exists:
                raise ValueError(
                    """
For some reason new_plotly.py was unable to communicate with the
...
{info}
new_plotly.py will attempt to start the local server process again the next time
...
""".format(
        ...
        # ### Try to be helpful ###
        # Status codes from /src/component/new_plotly-graph/constants.js in the
        # orca code base.
        # statusMsg: {
        #     400: 'invalid or malformed request syntax',
        #     522: client socket timeout
        #     525: 'new_plotly.js error',
        #     526: 'new_plotly.js version 1.11.0 or up required',
...
>>> new_plotly.io.orca.config.mapbox_access_token = 'pk.abc...'
