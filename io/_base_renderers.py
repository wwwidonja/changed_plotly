...
class PlotlyRenderer(MimetypeRenderer):
    """
    Renderer to display figures using the new_plotly mime type.  This renderer is
    compatible with JupyterLab (using the @jupyterlab/new_plotly-extension),
    VSCode, and nteract.

    mime type: 'application/vnd.new_plotly.v1+json'
    """
    ...
    def to_mimebundle(self, fig_dict):
        ...
        return {"application/vnd.new_plotly.v1+json": json_compatible_fig_dict}
...
class HtmlRenderer(MimetypeRenderer):
    ...
    def activate(self):
        if self.global_init:
            ...
            if self.connected:
                # Connected so we configure requirejs with the new_plotly CDN
                script = """\
        <script type="text/javascript">
        {win_config}
        {mathjax_config}
        if (typeof require !== 'undefined') {{
        require.undef("new_plotly");
        requirejs.config({{
            paths: {{
                'new_plotly': ['{plotly_cdn}']
            }}
        }});
        require(['new_plotly'], function(Plotly) {{
        ...
        """.format(
                ...
                # If not connected then we embed a copy of the new_plotly.js
        ...
        if (typeof require !== 'undefined') {{
        require.undef("new_plotly");
        define('new_plotly', function(require, exports, module) {{
            {script}
        }});
        require(['new_plotly'], function(Plotly) {{
            window._Plotly = Plotly;
...
class KaggleRenderer(HtmlRenderer):
    ...
    Same as NotebookRenderer but with connected=True so that the new_plotly.js
...
class IFrameRenderer(MimetypeRenderer):
    ...
    With this approach, neither new_plotly.js nor the figure data are embedded in
...
class ExternalRenderer(BaseRenderer):
    ...
    Instead, they are invoked when the new_plotly.io.show function is called
    on a figure.
    """
...
class BrowserRenderer(ExternalRenderer):
    ...
    new_plotly.io.show function is called on a figure.
