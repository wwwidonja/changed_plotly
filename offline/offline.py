""" Plotly Offline
    ...
    without connecting to a public or private new_plotly enterprise
    server.
"""
...
def download_plotlyjs(download_url):
    warnings.warn(
        ...
        next release. new_plotly.js is shipped with this module, it is no
...
def get_plotlyjs_version():
    """
    Returns the version of new_plotly.js that is bundled with new_plotly.py.
...
def get_plotlyjs():
    """
    Return the contents of the minified new_plotly.js library as a string.
    ...
    -------
    str
        Contents of the minified new_plotly.js library as a string

    Examples
    --------
    Here is an example of creating a standalone HTML report that contains
    two new_plotly figures, each in their own div.  The include_plotlyjs argument
    is set to False when creating the divs so that we don't include multiple
    copies of the new_plotly.js library in the output.  Instead, a single copy
    of new_plotly.js is included in a script tag in the html head element.

    >>> import new_plotly.graph_objs as go
    >>> from new_plotly.offline import plot, get_plotlyjs
    ...
    """
    path = os.path.join("package_data", "new_plotly.min.js")
    plotlyjs = pkgutil.get_data("new_plotly", path).decode("utf-8")
    return plotlyjs
...
def _get_jconfig(config=None):
    ...
    if config and isinstance(config, dict):
        ...
        # place yet to guarantee that the config options in new_plotly.py are up
        # to date
...
def init_notebook_mode(connected=False):
    """
    Initialize new_plotly.js in the browser if it hasn't been loaded into the DOM
    yet. This is an idempotent method and can and should be called from any
    offline methods that require new_plotly.js to be loaded into the notebook dom.

    Keyword arguments:

    connected (default=False) -- If True, the new_plotly.js library will be loaded
    from an online CDN. If False, the new_plotly.js library will be loaded locally
    from the new_plotly python package

    Use `connected=True` if you want your notebooks to have smaller file sizes.
    In the case where `connected=False`, the entirety of the new_plotly.js library
    ...
    # Trigger immediate activation of notebook. This way the new_plotly.js
...
def iplot(
    ...
    """
    Draw new_plotly graphs inside an IPython or Jupyter notebook

    figure_or_data -- a new_plotly.graph_objs.Figure or new_plotly.graph_objs.Data or
    ...
    validate (default=True) -- validate that all of the keys in the figure
                               are valid? omit if your version of new_plotly.js
    ...
    image (default=None |'png' |'jpeg' |'svg' |'webp') -- This parameter sets
        ...
        and more export options, consider using new_plotly.io.write_image. See
    ...
    Example:
    ```
    from new_plotly.offline import init_notebook_mode, iplot
    init_notebook_mode()
    ...
    ```
    from new_plotly.offline import iplot
...
def plot(
    ...
    """ Create a new_plotly graph locally as an HTML document or string.

    Example:
    ```
    from new_plotly.offline import plot
    import new_plotly.graph_objs as go
    ...
    link_text (default='Export to plot.ly') -- the text of export link
    validate (default=True) -- validate that all of the keys in the figure
        are valid? omit if your version of new_plotly.js has become outdated
    ...
    include_plotlyjs (True | False | 'cdn' | 'directory' | path - default=True)
        Specifies how the new_plotly.js library is included in the output html
        file or div string.

        If True, a script tag containing the new_plotly.js source code (~3MB)
        is included in the output.  HTML files generated with this option are
        fully self-contained and can be used offline.

        If 'cdn', a script tag that references the new_plotly.js CDN is included
        in the output. HTML files generated with this option are about 3MB
        smaller than those generated with include_plotlyjs=True, but they
        require an active internet connection in order to load the new_plotly.js
        library.

        If 'directory', a script tag is included that references an external
        new_plotly.min.js bundle that is assumed to reside in the same
        directory as the HTML file.  If output_type='file' then the
        new_plotly.min.js bundle is copied into the directory of the resulting
        HTML file. If a file named new_plotly.min.js already exists in the output
        ...
        require a copy of the new_plotly.min.js bundle in the same directory.
        This option is useful when many figures will be saved as HTML files in
        the same directory because the new_plotly.js source code will be included
        ...
        If False, no script tag referencing new_plotly.js is included. This is
        useful when output_type='div' and the resulting div string will be
        placed inside an HTML document that already loads new_plotly.js.  This
...
def plot_mpl(
    ...
    For more information on converting matplotlib visualizations to new_plotly
    graphs, call help(new_plotly.tools.mpl_to_plotly)

    For more information on creating new_plotly charts locally as an HTML document
    or string, call help(new_plotly.offline.plot)

    mpl_fig -- a matplotlib figure object to convert to a new_plotly graph

    Keyword arguments:
    resize (default=False) -- allow new_plotly to choose the figure size.
    ...
    output_type ('file' | 'div' - default 'file') -- if 'file', then
        ...
        Use 'div' if you are embedding these graphs in an HTML file with
        other graphs or HTML markup, like a HTML report or an website.
    include_plotlyjs (default=True) -- If True, include the new_plotly.js
        source code in the output file or string.
        Set as False if your HTML file already contains a copy of the new_plotly.js
        library.
    filename (default='temp-plot.html') -- The local filename to save the
        outputted chart to. If the filename already exists, it will be
        overwritten. This argument only applies if `output_type` is 'file'.
    ...
    Example:
    ```
    from new_plotly.offline import init_notebook_mode, plot_mpl
...
def iplot_mpl(
    ...
    """
    Convert a matplotlib figure to a new_plotly graph and plot inside an IPython
    notebook without connecting to an external server.

    To save the chart to Plotly Cloud or Plotly Enterprise, use
    `new_plotly.new_plotly.plot_mpl`.

    For more information on converting matplotlib visualizations to new_plotly
    graphs call `help(new_plotly.tools.mpl_to_plotly)`

    For more information on plotting new_plotly charts offline in an Ipython
    notebook call `help(new_plotly.offline.iplot)`

    mpl_fig -- a matplotlib.figure to convert to a new_plotly graph
    ...
    Example:
    ```
    from new_plotly.offline import init_notebook_mode, iplot_mpl
    import matplotlib.pyplot as plt
...
def enable_mpl_offline(
    ...
    Example:
    ```
    from new_plotly.offline import enable_mpl_offline
    import matplotlib.pyplot as plt
