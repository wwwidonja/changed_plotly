...
# Plotly mimetype
# ---------------
plotly_mimetype = "application/vnd.new_plotly.v1+json"
...
# Static Image
# ------------
# See new_plotly/tests/test_orca/test_image_renderers.py
...
def assert_requirejs(html):
    assert 'require(["new_plotly"]' in html


def assert_not_requirejs(html):
    assert 'require(["new_plotly"]' not in html
...
def test_repr_html(renderer):
    ...
    template = (
        ...
        '<div id="cd462b94-79ce-42a2-887f-2650a761a144" class="new_plotly-graph-div" '
...
def test_repr_mimebundle_mixed_renderer(fig1):
    ...
    assert set(fig1._repr_mimebundle_().keys()) == set(
        {"application/vnd.new_plotly.v1+json", "text/html"}
    )
