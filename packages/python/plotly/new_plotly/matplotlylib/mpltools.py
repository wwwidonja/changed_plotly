...
A module for converting from mpl language to new_plotly language.
...
def convert_dash(mpl_dash):
    """Convert mpl line symbol to new_plotly line symbol and return symbol."""
...
def convert_symbol(mpl_symbol):
    """Convert mpl marker symbol to new_plotly symbol and return symbol."""
...
def convert_va(mpl_va):
    ...
    if mpl_va in VA_MAP:
        ...
        return None  # let new_plotly figure it out!


def convert_x_domain(mpl_plot_bounds, mpl_max_x_bounds):
    """Map x dimension of current plot to new_plotly's domain space.

    The bbox used to locate an axes object in mpl differs from the
    method used to locate axes in new_plotly. The mpl version locates each
    ...
    new_plotly such that the domain will always be [0, 0, 1, 1]
    ...
    mpl to new_plotly would place axes appropriately. However,
...
def convert_y_domain(mpl_plot_bounds, mpl_max_y_bounds):
    """Map y dimension of current plot to new_plotly's domain space.
...
def display_to_paper(x, y, layout):
    """Convert mpl display coordinates to new_plotly paper coordinates.
    ...
    the entire new_plotly axes space where the bottom-left of the bottom-left
...
def get_axes_bounds(fig):
    ...
    In new_plotly, the axes area (where all subplots go) is always specified with
    ...
    box is then used to map mpl axes domains to new_plotly axes domains.
...
def make_bar(**props):
    ...
    dictionary that needs to get passed to new_plotly as a data dictionary. That
    happens in PlotlyRenderer in that class's draw_bar method. In other
    words, this dictionary describes a SINGLE bar, whereas, new_plotly will
