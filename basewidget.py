...
class BaseFigureWidget(BaseFigure, widgets.DOMWidget):
    ...
    _view_module = Unicode("jupyterlab-new_plotly").tag(sync=True)
    _view_module_version = Unicode(__frontend_version__).tag(sync=True)

    _model_name = Unicode("FigureModel").tag(sync=True)
    _model_module = Unicode("jupyterlab-new_plotly").tag(sync=True)
    ...
    def _handler_js2py_relayout(self, change):
        ...
        if "lastInputTime" in relayout_data:
            # Remove 'lastInputTime'. Seems to be an internal new_plotly
    ...
    def _display_frames_error():
        ...
        """
        msg = """
Frames are not supported by the new_plotly.graph_objs.FigureWidget class.
Note: Frames are supported by the new_plotly.graph_objs.Figure class"""
